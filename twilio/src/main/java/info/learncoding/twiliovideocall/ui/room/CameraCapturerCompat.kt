...
class CameraCapturerCompat(
    ...
    override fun startCapture(width: Int, height: Int, framerate: Int) {
        cameraCapturer?.startCapture(width, height, framerate) ?: camera2Capturer?.startCapture(
            width,
            height,
            framerate
        )
    ...
    override fun isScreencast() =
        cameraCapturer?.isScreencast ?: camera2Capturer?.isScreencast ?: false
    ...
    fun switchCamera() {
        if (frontCameraId != null && backCameraId != null) {
            ...
            "Front and back cameras need to both be available in order to switch between them"
        )
    }
    ...
    companion object {
            ...
            return if (Camera2Capturer.isSupported(context)) {
                Camera2Enumerator(context).getFrontAndBackCameraIds(context)?.let { cameraIds ->
                    val cameraCapturer = Camera2Capturer(
                        context, cameraIds.first
                            ?: cameraIds.second ?: ""
                    )
                    CameraCapturerCompat(
                        cameraIds.first,
                        cameraIds.second,
                        camera2Capturer = cameraCapturer
                    )
                }
            } else {
                Camera1Enumerator().getFrontAndBackCameraIds(context, isCamera2 = false)
                    ?.let { cameraIds ->
                        val cameraCapturer = CameraCapturer(
                            context, cameraIds.first ?: cameraIds.second
                            ?: "", getCameraListener()
                        )
                        CameraCapturerCompat(
                            cameraIds.first,
        ...
        private fun CameraEnumerator.getFrontAndBackCameraIds(
            context: Context,
            isCamera2: Boolean = true
        ): Pair<String?, String?>? {
            val cameraIds = deviceNames.find {
                isFrontFacing(it) && isCameraIdSupported(
                    isCamera2,
                    context,
                    it
                )
        ...
        private fun isCameraIdSupported(context: Context, cameraId: String): Boolean {
            ...
            val cameraCharacteristics: CameraCharacteristics = try {
                ...
                Log.e("CameraCapturerCompat", "Error", e)
                return false
            }
            /*
